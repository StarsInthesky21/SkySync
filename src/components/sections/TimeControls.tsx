...
function formatTime(date: Date) { return date.toISOString().slice(11, 16); }
...
export function TimeControls({ voiceGuideEnabled, setVoiceGuideEnabled, onStatus }: Props) {
  ...
  function handleApply() {
    if (!DATE_REGEX.test(dateInput.trim()) || !TIME_REGEX.test(timeInput.trim())) {
      onStatus("Invalid date or time format"); return;
    }
    const parsed = new Date(`${dateInput.trim()}T${timeInput.trim()}:00.000Z`);
    if (Number.isNaN(parsed.getTime())) { onStatus("Invalid date or time"); return; }
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.buttonRow}>
        <Pressable style={({ pressed }) => [styles.secondary, pressed && styles.secondaryPressed]} onPress={handleApply}>
          <Text style={styles.secondaryText}>Apply</Text>
        </Pressable>
        <Pressable style={({ pressed }) => [styles.primary, pressed && styles.primaryPressed]} onPress={handleNow}>
