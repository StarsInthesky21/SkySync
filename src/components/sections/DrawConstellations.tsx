    ...
    <View style={styles.card}>
      <View style={styles.header}>
        ...
        <Switch
          ...
          onValueChange={(v) => {
            setDrawModeEnabled(v);
            onStatus(v ? "Tap stars to draw" : "Draw mode off");
          }}
          thumbColor={drawModeEnabled ? colors.accent : "#aaa"}
          trackColor={{ false: "rgba(255,255,255,0.15)", true: colors.glow }}
          accessibilityLabel="Toggle draw constellation mode"
        />
      </View>
      <Text style={styles.caption} accessibilityLiveRegion="polite">
        {draftConstellationIds.length} stars selected
      </Text>
      <TextInput
        value={titleInput}
        ...
        maxLength={40}
        accessibilityLabel="Name for your custom constellation"
      />
      <View style={styles.buttonRow}>
        <Pressable
          style={({ pressed }) => [styles.secondary, pressed && styles.secondaryPressed]}
          onPress={clearDraftConstellation}
          accessibilityRole="button"
          accessibilityLabel="Clear selected stars"
        >
        ...
        <Pressable
          style={({ pressed }) => [
            styles.primary,
            draftConstellationIds.length < 2 && styles.disabled,
            pressed && styles.primaryPressed,
          ]}
          onPress={async () => {
            if (draftConstellationIds.length < 2) {
              onStatus("Select at least 2 stars");
              return;
            }
