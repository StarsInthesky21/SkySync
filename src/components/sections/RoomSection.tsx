  ...
  return (
    <View style={styles.card}>
        ...
        <View style={styles.browserSection}>
          ...
          {rooms.length === 0 ? (
            <View style={styles.emptyBrowser}>
              <Text style={styles.emptyBrowserText}>No public rooms available. Create one to get started!</Text>
            </View>
          ) : (
            <FlatList
              data={rooms}
              ...
              renderItem={({ item }: { item: SkyRoom }) => {
                ...
                return (
                  <Pressable
                    style={({ pressed }) => [styles.roomCard, isCurrentRoom && styles.roomCardActive, pressed && { opacity: 0.8 }]}
      ...
      <View style={styles.voiceRow} accessibilityRole="switch">
        ...
        <Switch
          value={callActive}
          onValueChange={(v) => { setCallActive(v); onStatus(v ? "Room marked voice-ready" : "Voice-ready flag cleared"); }}
      ...
      <View style={styles.buttonRow}>
        <Pressable style={({ pressed }) => [styles.secondary, pressed && styles.secondaryPressed]} onPress={handleJoin} accessibilityRole="button">
          <Text style={styles.secondaryText}>Join</Text>
        </Pressable>
        <Pressable style={({ pressed }) => [styles.primary, pressed && styles.primaryPressed]} onPress={handleCreate} accessibilityRole="button">
...
const styles = StyleSheet.create({
  ...
  browseBtn: { borderRadius: radius.pill, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: "rgba(115,251,211,0.1)", borderWidth: 1, borderColor: "rgba(115,251,211,0.2)" },
  browseBtnText: { color: colors.accent, fontWeight: "700", fontSize: fontSize.xs },
  // Room browser
  browserSection: { marginBottom: 12, gap: 8 },
  browserTitle: { color: colors.textMuted, fontSize: fontSize.xs, fontWeight: "600", textTransform: "uppercase", letterSpacing: 1 },
  emptyBrowser: { borderRadius: radius.md, padding: 16, backgroundColor: "rgba(255,255,255,0.02)", alignItems: "center" },
  emptyBrowserText: { color: colors.textDim, fontSize: fontSize.xs, textAlign: "center" },
  roomCard: { borderRadius: radius.lg, padding: 12, backgroundColor: "rgba(255,255,255,0.03)", marginTop: 6, borderWidth: 1, borderColor: "rgba(255,255,255,0.04)" },
  ...
  roomBadge: { borderRadius: radius.pill, paddingHorizontal: 8, paddingVertical: 3, backgroundColor: "rgba(255,255,255,0.06)" },
  ...
  voiceIndicator: { borderRadius: radius.pill, paddingHorizontal: 6, paddingVertical: 2, backgroundColor: "rgba(255,177,95,0.1)" },
  voiceIndicatorText: { color: colors.accentWarm, fontSize: 10, fontWeight: "600" },
  // Current room
  currentRoom: { borderRadius: radius.md, padding: 10, backgroundColor: "rgba(115,251,211,0.05)", borderWidth: 1, borderColor: "rgba(115,251,211,0.1)", marginBottom: 8 },
