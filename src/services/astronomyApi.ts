...
function computePlanetPositions(date: Date): PlanetPosition[] {
  ...
  const earthV =
    ...
    RAD;
  const earthR = (1.0 * (1 - 0.01671 * 0.01671)) / (1 + 0.01671 * Math.cos(earthV * DEG));
  ...
  return PLANETS.map((planet) => {
    ...
    const trueAnomaly =
      2 *
      Math.atan2(Math.sqrt(1 + e) * Math.sin((E * DEG) / 2), Math.sqrt(1 - e) * Math.cos((E * DEG) / 2)) *
      RAD;
    ...
    const xH =
      r *
      (Math.cos(Omega * DEG) * Math.cos((helioLon - Omega) * DEG) -
        Math.sin(Omega * DEG) * Math.sin((helioLon - Omega) * DEG) * Math.cos(I * DEG));
    const yH =
      r *
      (Math.sin(Omega * DEG) * Math.cos((helioLon - Omega) * DEG) +
        Math.cos(Omega * DEG) * Math.sin((helioLon - Omega) * DEG) * Math.cos(I * DEG));
    ...
    const phaseAngle =
      Math.acos((r * r + distanceAU * distanceAU - earthR * earthR) / (2 * r * distanceAU)) * RAD;
...
function getConstellationFromRA(ra: number, _dec: number): string {
...
function computeUpcomingEvents(date: Date): AstroEvent[] {
  ...
  const allEvents: AstroEvent[] = [
    // Meteor showers (recurring annually)
    {
      ...
      date: `${year}-01-04`,
      description: "Up to 120 meteors per hour. Best viewed after midnight. Radiant in Bootes.",
      type: "meteor_shower",
      importance: "high",
    },
    {
      ...
      date: `${year}-04-22`,
      description: "18 meteors per hour from dust of Comet Thatcher. Radiant near Vega in Lyra.",
      type: "meteor_shower",
      importance: "medium",
    },
    {
      id: `eta-aquariids-${year}`,
      title: "Eta Aquariids",
      date: `${year}-05-06`,
      description: "Up to 60 meteors per hour. Debris from Halley's Comet. Best for southern observers.",
      type: "meteor_shower",
      importance: "high",
    },
    {
      ...
      date: `${year}-08-12`,
      description: "Up to 100 fast, bright meteors per hour. The most popular shower of the year.",
      type: "meteor_shower",
      importance: "high",
    },
    {
      ...
      date: `${year}-10-21`,
      description: "20 meteors per hour from Halley's Comet debris. Fast meteors with persistent trains.",
      type: "meteor_shower",
      importance: "medium",
    },
    {
      ...
      date: `${year}-11-17`,
      description: "15 meteors per hour. Occasional storm years with thousands of meteors.",
      type: "meteor_shower",
      importance: "medium",
    },
    {
      ...
      date: `${year}-12-14`,
      description: "Up to 150 multicolored meteors per hour. King of meteor showers.",
      type: "meteor_shower",
      importance: "high",
    },
    ...
    // Equinoxes & Solstices
    {
      ...
      date: `${year}-03-20`,
      description: "Day and night are nearly equal. Start of astronomical spring in the Northern Hemisphere.",
      type: "equinox",
      importance: "medium",
    },
    {
      ...
      date: `${year}-06-21`,
      description: "Longest day of the year in Northern Hemisphere. Sun reaches highest point in sky.",
      type: "solstice",
      importance: "medium",
    },
    {
      ...
      date: `${year}-09-22`,
      description: "Day and night nearly equal again. Start of astronomical autumn.",
      type: "equinox",
      importance: "medium",
    },
    {
      ...
      date: `${year}-12-21`,
      description: "Shortest day, longest night. Best dark-sky viewing of the year.",
      type: "solstice",
      importance: "medium",
    },
    ...
    {
      id: `lunar-eclipse-${year}-mar`,
      title: "Total Lunar Eclipse",
      date: `${year}-03-14`,
      description:
        "The Moon passes through Earth's shadow, turning deep red. Visible across Americas and Europe.",
      type: "eclipse",
      importance: "high",
    },
    {
      id: `solar-eclipse-${year}-sep`,
      title: "Partial Solar Eclipse",
      date: `${year}-09-07`,
      description:
        "The Moon partially covers the Sun. Visible from southern hemisphere. Never look directly at the Sun.",
      type: "eclipse",
      importance: "high",
    },
    ...
    // Planetary events
    {
      ...
      date: `${year}-12-07`,
      description: "Jupiter at its closest to Earth and fully lit by the Sun. Best viewing all year.",
      type: "opposition",
      importance: "high",
    },
    {
      ...
      date: `${year}-09-21`,
      description: "Saturn at its brightest. Rings beautifully visible through a telescope.",
      type: "opposition",
      importance: "high",
    },
    {
      id: `mars-conj-${year}`,
      title: "Mars Conjunction",
      date: `${year}-01-09`,
      description: "Mars passes close to the Sun from our perspective. It will re-emerge as a morning star.",
      type: "conjunction",
      importance: "low",
    },
  ];
